...
import { FileText, Clock, Eye, ExternalLink, ShieldAlert, Inbox, UserPlus, Zap, Loader2 } from 'lucide-react';
import { Link } from 'react-router-dom';
import { useExpediteReviewMutation } from '../../api/baseApi';
import { useGetMyPointsQuery } from '../../api/baseApi';
import toast from 'react-hot-toast';

const ArticleRow = ({ article, onWithdraw, onAssignReviewer, onExpediteReview, userPoints }) => (
    ...
    <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
      <div className="flex-1 space-y-3">
        <div className="flex items-center gap-3">
          <span className="px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-widest border bg-orange-50 text-orange-600 border-orange-100">
            Under Review
          </span>
          {article.is_priority && (
      ...
      <div className="flex items-center gap-2 shrink-0">
        ...
        <button onClick={() => onWithdraw(article)} className="ml-2 px-6 py-2.5 rounded-xl text-[10px] font-bold uppercase tracking-widest border-2 border-red-100 text-red-500 hover:bg-red-50 transition-all flex items-center gap-2">
...
const UnderReviewTab = ({ articles, isLoading, onWithdraw, onAssignReviewer }) => {
  const { data: myPoints } = useGetMyPointsQuery();
  const [expediteReview, { isLoading: isExpediting }] = useExpediteReviewMutation();
  
  const userPoints = myPoints?.total || 0;

  const handleExpediteReview = async (articleId) => {
    if (userPoints < 50) {
      toast.error('Insufficient points. You need 50 points to expedite review.');
      return;
    }

    try {
      await expediteReview(articleId).unwrap();
      toast.success('Review expedited successfully! Your article is now marked as priority.');
    } catch (error) {
  ...
  if (!articles || articles.length === 0) return (
    <div className="bg-white p-20 rounded-[3rem] border border-gray-50 text-center space-y-6 animate-in zoom-in duration-500">
      ...
      <div className="space-y-2">
        <h3 className="text-2xl font-serif font-bold text-primary tracking-tight">No Manuscripts Under Review</h3>
        <p className="text-gray-400 font-medium max-w-sm mx-auto italic">Your submitted work is currently being evaluated by our peer review board. New updates will appear here.</p>
      </div>
    </div>
  );
      ...
      {userPoints >= 1000 && (
        <div className="bg-gradient-to-r from-amber-50 to-yellow-50 p-6 rounded-[2rem] border border-amber-200 flex items-center gap-4">
          ...
          <div className="flex-1">
            <h4 className="text-lg font-bold text-amber-900">Publication Cost Support Eligible</h4>
            <p className="text-sm text-amber-700">You are eligible for full publication cost support on your submissions!</p>
          </div>
        </div>
      )}
      <div className="space-y-6">
